...
static int hammergui_validate(int dosetup){
    if(dosetup && !hammergui_sink && (hammergui_class || hammergui_setup())){
        if(ps_hashhammergui->s_thing)
            ...
            pd_bind((t_pd *)hammergui_sink, ps_hashhammergui);  // never unbound <= bug?
        }
    }
...
void hammergui_unbindmouse(t_pd *master){
    if(hammergui_validate(0) && hammergui_mousevalidate(0) && hammergui_sink->g_psmouse->s_thing){
...
void hammergui_unbindfocus(t_pd *master){
    if(hammergui_validate(0) && hammergui_focusvalidate(0) && hammergui_sink->g_psfocus->s_thing){
...
void hammergui_unbindvised(t_pd *master){
    if(hammergui_validate(0) && hammergui_visedvalidate(0) && hammergui_sink->g_psvised->s_thing){
