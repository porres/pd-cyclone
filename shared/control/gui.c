...
#ifdef PDL2ORK
extern void glist_getnextxy(t_glist *gl, int *xpix, int *ypix);

static void hammergui__up(t_hammergui *snk, t_floatarg f);

static void hammergui_poll_tick(t_hammergui *snk)
{
    hammergui_getscreen();
    if (snk->g_pollclock)
        clock_delay(snk->g_pollclock, 20);
}

static void hammergui_pdl2ork_list(t_hammergui *snk, t_symbol *s, int ac, t_atom *av)
{
    s = NULL;
    if (ac == 4 && av[0].a_type == A_FLOAT) {
        // #legacy_mouseclick [down, button, x, y]
        float down = av[0].a_w.w_float;
        hammergui__up(snk, (down == 0.0 ? 1.0 : 0.0));
    }
    else if (ac == 2 && av[0].a_type == A_FLOAT && av[1].a_type == A_FLOAT) {
        // #legacy_mousemotion [x, y]
        snk->g_mousex = av[0].a_w.w_float;
        snk->g_mousey = av[1].a_w.w_float;
    }
}
#endif
...
#ifdef PDL2ORK
        pd_bind((t_pd *)hammergui_sink, gensym("#legacy_mouseclick"));
        pd_bind((t_pd *)hammergui_sink, gensym("#legacy_mousemotion"));
        class_addlist(hammergui_class, hammergui_pdl2ork_list);
#else
