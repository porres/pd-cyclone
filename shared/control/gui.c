...
static void hammergui_anything(t_hammergui *snk, t_symbol *s, int ac, t_atom *av){
    // Dummy method to filter gui messages to keep Pd happy in `gray periods'
...
static int hammergui_setup(void){
...
#ifdef HAMMERGUI_DEBUG
        fprintf(stderr, "'%s' already registered as the global hammergui sink \n",
            (cname ? cname : "???"));
#endif
    ...
    hammergui_class = class_new(ps__hammergui, 0, 0, sizeof(t_hammergui),
        CLASS_PD | CLASS_NOINLET, 0);
    class_addanything(hammergui_class, hammergui_anything);
    class_addmethod(hammergui_class, (t_method)hammergui__remouse, gensym("_remouse"), 0);
    class_addmethod(hammergui_class, (t_method)hammergui__refocus, gensym("_refocus"), 0);
    class_addmethod(hammergui_class, (t_method)hammergui__revised, gensym("_revised"), 0);
    class_addmethod(hammergui_class, (t_method)hammergui__up, ps__up, A_FLOAT, 0);
...
static int hammergui_validate(int dosetup){
    if (dosetup && !hammergui_sink && (hammergui_class || hammergui_setup())){
        if(ps_hashhammergui->s_thing)
            ...
            pd_bind((t_pd *)hammergui_sink, ps_hashhammergui); // never unbound
        }
    }
...
static int hammergui_pollvalidate(int dosetup){
    if (dosetup && !hammergui_sink->g_pspoll){
        ...
        pd_bind((t_pd *)hammergui_sink, hammergui_sink->g_pspoll); // never unbound
    }
