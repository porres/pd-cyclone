...
static void Borax_delta(t_Borax *x){ /* CHECKME first note */
...
static void Borax_float(t_Borax *x, t_float f){
    if((int)f == f){
        int pitch;
        int index;
        if (pitch < 0 || pitch >= BORAX_MAXVOICES) // CHECKME pitch range, complaints
            return;
        ...
        pd_error(x, "[borax]: doesn't understand \"noninteger float\"");
    }
}
