...
#define PAK_MINSLOTS  2
#define PAK_INISIZE   4  /* LATER rethink (useful only in multiatom mode) */
...
typedef struct _pak_proxy
{
    ...
    t_atom      *p_message;
    t_atom       p_messini[PAK_INISIZE];
} t_pak_proxy;

static t_class *pak_class;
static t_class *pak_proxy_class;

static void pak_doit(t_pak *x)
{
    t_pak_proxy **p = (t_pak_proxy **)x->x_proxies;
    int i = x->x_nslots;
    p = (t_pak_proxy **)x->x_proxies;
    i = x->x_nslots;
    while (i--)
    {
...
static void pak_arm(t_pak *x)
{
    if (x->x_delay <= 0) pak_doit(x);
    else clock_delay(x->x_clock, x->x_delay);
}

static void pak_proxy_bang(t_pak_proxy *x)
{
    pak_arm(x->p_master);  /* CHECKED: bang in any inlet works in this way */
}

static void pak_proxy_dofloat(t_pak_proxy *x, t_float f, int doit)
{
    x->p_selector = &s_float;
    x->p_float = f;
    x->p_natoms = 0;  /* defensive */
    if (doit) pak_arm(x->p_master);
}

static void pak_proxy_float(t_pak_proxy *x, t_float f)
{
    pak_proxy_dofloat(x, f, 1);
}

static void pak_proxy_dosymbol(t_pak_proxy *x, t_symbol *s, int doit)
{
...
static void pak_proxy_symbol(t_pak_proxy *x, t_symbol *s)
{
    pak_proxy_dosymbol(x, s, 1);
}

static void pak_proxy_dopointer(t_pak_proxy *x, t_gpointer *gp, int doit)
{
...
static void pak_proxy_pointer(t_pak_proxy *x, t_gpointer *gp)
{
    pak_proxy_dopointer(x, gp, 1);
}

/* CHECKED: the slots fire in right-to-left order,
   but they trigger only once (refman error) */
static void pak_distribute(t_pak *x, int startid,
			     t_symbol *s, int ac, t_atom *av, int doit)
{
	...
	id = x->x_nslots;
    ap += id - startid;
    pp = (t_pak_proxy **)(x->x_proxies + id);
    if (s) ap--;
    while (ap-- > av)
    {
	pp--;
	if (ap->a_type == A_FLOAT)
	    pak_proxy_dofloat(*pp, ap->a_w.w_float, 0);
	else if (ap->a_type == A_SYMBOL)
	    pak_proxy_dosymbol(*pp, ap->a_w.w_symbol, 0);
	else if (ap->a_type == A_POINTER)
	    pak_proxy_dopointer(*pp, ap->a_w.w_gpointer, 0);
    }
    if (s)
	pak_proxy_dosymbol((t_pak_proxy *)x->x_proxies[startid], s, 0);
    if (doit) pak_arm(x);
}

static void pak_proxy_domultiatom(t_pak_proxy *x,
				    int ac, t_atom *av, int doit)
{
    if (ac > x->p_size)
    {
	/* LATER consider using PAK_MAXSIZE (and warning if exceeded) */
	x->p_message = grow_nodata(&ac, &x->p_size, x->p_message,
				   PAK_INISIZE, x->p_messini,
				   sizeof(*x->p_message));
    }
    x->p_natoms = ac;
...
static void pak_proxy_dolist(t_pak_proxy *x, int ac, t_atom *av, int doit)
{
    if (x->p_master->x_multiatom)
    {
	x->p_selector = &s_list;
	pak_proxy_domultiatom(x, ac, av, doit);
    }
    else pak_distribute(x->p_master, x->p_id, 0, ac, av, doit);
}

static void pak_proxy_list(t_pak_proxy *x,
			     t_symbol *s, int ac, t_atom *av)
{
    pak_proxy_dolist(x, ac, av, 1);
}

static void pak_proxy_doanything(t_pak_proxy *x,
				   t_symbol *s, int ac, t_atom *av, int doit)
{
    if (x->p_master->x_multiatom)
    {
	...
	if (s == &s_symbol)
	{
		...
		pak_proxy_dosymbol(x, av->a_w.w_symbol, doit);
	    else
		pak_proxy_dosymbol(x, &s_symbol, doit);
	}
	else
	{
	    x->p_selector = s;
    ...
    else pak_distribute(x->p_master, x->p_id, s, ac, av, doit);
}

static void pak_proxy_anything(t_pak_proxy *x,
				 t_symbol *s, int ac, t_atom *av)
{
    pak_proxy_doanything(x, s, ac, av, 1);
}

static void pak_proxy_set(t_pak_proxy *x,
			    t_symbol *s, int ac, t_atom *av)
{
    if (ac)
    {
	...
	{
	    if (ac > 1)
		pak_proxy_dolist(x, ac, av, 0);
	    else
		pak_proxy_dofloat(x, av->a_w.w_float, 0);
	}
	else if (av->a_type == A_SYMBOL)
	    /* CHECKED: no tests for 'set float ...' and 'set list...' --
	       the parsing is made in an output routine */
	    pak_proxy_doanything(x, av->a_w.w_symbol, ac-1, av+1, 0);
	else if (av->a_type == A_POINTER)
	    pak_proxy_dopointer(x, av->a_w.w_gpointer, 0);
    }
...
{
    pak_proxy_bang((t_pak_proxy *)x->x_proxies[0]);
}

static void pak_float(t_pak *x, t_float f)
{
    pak_proxy_dofloat((t_pak_proxy *)x->x_proxies[0], f, 1);
}

static void pak_symbol(t_pak *x, t_symbol *s)
{
    pak_proxy_dosymbol((t_pak_proxy *)x->x_proxies[0], s, 1);
}

static void pak_pointer(t_pak *x, t_gpointer *gp)
{
    pak_proxy_dopointer((t_pak_proxy *)x->x_proxies[0], gp, 1);
}

static void pak_list(t_pak *x, t_symbol *s, int ac, t_atom *av)
{
    pak_proxy_dolist((t_pak_proxy *)x->x_proxies[0], ac, av, 1);
}

static void pak_anything(t_pak *x, t_symbol *s, int ac, t_atom *av)
{
    pak_proxy_doanything((t_pak_proxy *)x->x_proxies[0], s, ac, av, 1);
}

static void pak_set(t_pak *x, t_symbol *s, int ac, t_atom *av)
{
    pak_proxy_set((t_pak_proxy *)x->x_proxies[0], s, ac, av);
}
...
static void pak_free(t_pak *x)
{
	...
	while (i--)
	{
	    t_pak_proxy *y = (t_pak_proxy *)x->x_proxies[i];
...
static void *pak_new(t_symbol *s, int ac, t_atom *av)
{
    t_pak *x;
    int i, nslots, nproxies = PAK_MINSLOTS;
    int multiatom = 0;
    t_float delay = 0;
    t_pd **proxies;
    t_outlet **outs;
    ...
    if (nproxies < PAK_MINSLOTS)
	nproxies = PAK_MINSLOTS;
    if (!(proxies = (t_pd **)getbytes(nproxies * sizeof(*proxies))))
	return (0);
    for (nslots = 0; nslots < nproxies; nslots++)
	if (!(proxies[nslots] = pd_new(pak_proxy_class))) break;
    if (nslots < PAK_MINSLOTS
    ...
    for (i = 0; i < nslots; i++)
    {
	t_pak_proxy *y = (t_pak_proxy *)proxies[i];
	y->p_master = x;
	y->p_id = i;
...
void pak_setup(void)
{
    ...
    class_addmethod(pak_class, (t_method)pak_set,
    ...
    class_addmethod(pak_proxy_class, (t_method)pak_proxy_set,
		    gensym("set"), A_GIMME, 0);
}
