...
void abs_dsp(t_abs *x, t_signal **sp)
{  /* #ifdef ABS_TEST
...
void abs_tilde_setup(void)
{
    abs_class = class_new(gensym("cyclone/abs~"),
        (t_newmethod)abs_new, 0, sizeof(t_abs), CLASS_DEFAULT, 0);
    class_addmethod(abs_class, nullfn, gensym("signal"), 0);
    class_addmethod(abs_class, (t_method) abs_dsp, gensym("dsp"), 0);
/* #ifdef ABS_TEST
